 ...
 *
 *  librato : {
 *    api   : API host and port (default: 'https://metrics-api.librato.com')
 *    apiPath: API URL path (default: '/v1/metrics')
...
var debug;
var api, apiPath, email, token, period, sourceName;
...
{ 
  if (debug) { 
      util.log('Librato payload = ' + payload);
...
exports.init = function librato_init(startup_time, config, events)
{
  ...
  if (config.librato) {
    ...
    apiPath = config.librato.apiPath; 
  ...
  if (email && token) {
    ...
    util.log("Warning: Possible bad configuration for Librato Metrics backend - email or token is missing");
  }
